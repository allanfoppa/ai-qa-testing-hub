...
const projectConfig = projects.find(p => p.id === 'rick-and-morty-graphs-and-stuff');

test.describe('Rick and Morty Dashboard E2E', () => {

  test.beforeEach(async ({ page }) => {
    // Navigate using the URL from our Hub's registry
    await page.goto(projectConfig?.baseUrl || 'http://localhost:3000');
  });

  test('should load the dashboard and display character cards', async ({ page }) => {
  ...
  test('should filter characters when using the search bar', async ({ page }) => {
